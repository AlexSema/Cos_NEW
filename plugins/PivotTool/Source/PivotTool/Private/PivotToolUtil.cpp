...
bool FPivotToolUtil::BakeStaticMeshActorsPivotOffsetToRawMesh(AStaticMeshActor* InStaticMeshActor, const FPivotToolOptions& InOptions, bool bDuplicate, bool bSilentMode)
{
	...
	if (InStaticMeshActor)
	{
		...
		{	
			...
			// Keep Referecning Actors InPlace
			if (bKeepActorsInPlace)
			{
				...
				{
					AActor* Actor = *It;
					if (!Actor || Actor->IsPendingKillPending() || InStaticMeshActor == Actor)
					{
						continue;
					}
...
void FPivotToolUtil::TransformRawMeshVertexData(const FTransform& InTransform, FRawMesh& OutRawMesh, const FVector& InBuildScale3D, bool bFreezeBuildScale)
{
...
void FPivotToolUtil::TransformMeshVertexData(const FTransform& InTransform, FMeshDescription* OutMesh, const FVector& InBuildScale3D, bool bFreezeBuildScale /*= false*/)
{
	...
	if (bHasBuildScale)
	{
		FTransform3d ToBake(TQuaternion<double>::Identity(), FVector3<double>::Zero(), FVector3<double>(InBuildScale3D));
		MeshAdapterTransforms::ApplyTransform(EditableMeshDescAdapter, ToBake);
	}

	{
		FTransform3d ToBake(TQuaternion<double>(InTransform.GetRotation()), FVector3<double>(InTransform.GetLocation()), FVector3<double>(InTransform.GetScale3D()));
		MeshAdapterTransforms::ApplyTransform(EditableMeshDescAdapter, ToBake);
	}

	if (bHasBuildScale)
	{
		FTransform3d ToBake(TQuaternion<double>::Identity(), FVector3<double>::Zero(), FVector3<double>(FVector::OneVector / InBuildScale3D));
		MeshAdapterTransforms::ApplyTransform(EditableMeshDescAdapter, ToBake);
	}
}
